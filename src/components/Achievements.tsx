...
const Achievements = () => {
  ...
  return (
    <section ref={sectionRef} id="achievements" className="py-24 bg-slate-800 relative">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            {achievements.map((achievement, index) => (
              <div
                key={index}
                className="bg-slate-700 border border-slate-600 rounded-lg p-8 card-hover scale-in-on-scroll"
                style={{ animationDelay: `${index * 0.2}s` }}
              >
                <div className="flex items-start gap-6 mb-6">
                  <div className="p-4 bg-slate-600 rounded-lg">
                    <achievement.icon className="text-slate-300" size={28} />
                  </div>
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="text-slate-300 font-medium">{achievement.event}</span>
                    </div>
                    <div className="flex items-center gap-3 mb-2">
                      <Calendar className="text-slate-400" size={16} />
                      <span className="text-slate-400">{achievement.year}</span>
                    </div>
                    <p className="text-slate-400 mb-4">{achievement.organization}</p>
                  </div>
                </div>

                <p className="text-slate-300 leading-relaxed mb-6">{achievement.description}</p>

                <div className="flex items-center gap-3 mb-6">
                  <div className="p-2 bg-slate-600 rounded-lg">
                    <Edit3 className="text-slate-300" size={16} />
                  </div>
                  <span className="text-slate-300 font-medium">Role: {achievement.role}</span>
                </div>
                  ...
                  <ul className="space-y-2">
                    {achievement.details.map((detail, detailIndex) => (
                      <li key={detailIndex} className="flex items-start gap-3">
                        ...
                        <span className="text-slate-300 leading-relaxed text-sm">{detail}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <button
                  ...
                  className="w-full border border-slate-600 text-slate-300 px-6 py-3 rounded-lg font-medium hover:border-slate-500 hover:bg-slate-600 transition-all duration-200 flex items-center justify-center gap-2 text-sm"
                >
      ...
      {modalOpen && selectedImage && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-80 flex items-center justify-center px-4 transition-opacity duration-300">
          <div className="bg-slate-800 rounded-xl max-w-4xl w-full relative shadow-xl overflow-hidden transform scale-100 opacity-100 transition-all duration-300 ease-out">
            <button
              onClick={closeModal}
              className="absolute top-4 right-4 border border-slate-600 rounded-full p-2 
                         bg-slate-700/80 text-slate-300 
                         hover:bg-slate-600 shadow-lg backdrop-blur 
