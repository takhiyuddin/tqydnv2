...
const Portfolio = () => {
  ...
  return (
    <section ref={sectionRef} id="portfolio" className="py-24 bg-black relative">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((project, index) => (
              <div
                key={index}
                className="portfolio-card bg-gray-900 border border-gray-800 rounded-lg overflow-hidden scale-in-on-scroll group"
              ...
              >
                ...
                <div className="p-6 relative">
                  {/* Animated Border */}
                  <div className="absolute top-0 left-0 w-0 h-0.5 bg-gradient-to-r from-gray-600 to-gray-400 group-hover:w-full transition-all duration-500"></div>
                  
                  <button
                    onClick={() => handleOpenBlog(project)}
                    className="portfolio-title text-lg font-medium text-white mb-3 hover:text-gray-300 transition-all duration-300 text-left w-full group-hover:transform group-hover:-translate-y-1"
                  >
                    {project.title}
                  </button>
                  
                  <p className="text-gray-400 mb-4 leading-relaxed transform group-hover:-translate-y-1 transition-transform duration-300 delay-75">
                    {project.description}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-2 transform group-hover:-translate-y-1 transition-transform duration-300 delay-100">
                    {project.tags.map((tag, tagIndex) => (
                      <span
                        key={tagIndex}
                        className="portfolio-tag px-3 py-1 bg-gray-800 text-gray-400 rounded-full text-xs font-medium hover:bg-gray-700 hover:scale-105 transition-all duration-200"
                  ...
                  {/* Date and Role Info */}
                  <div className="mt-4 pt-4 border-t border-gray-800 flex items-center justify-between text-xs text-gray-500 transform translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 delay-150">
      ...
      {isPopupOpen && selectedProject && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 animate-fade-in">
          <div className="bg-gray-900 rounded-xl sm:rounded-2xl w-full max-w-5xl max-h-[98vh] sm:max-h-[95vh] shadow-2xl animate-scale-in transform overflow-hidden border border-gray-800">
            ...
            {/* Responsive Header */}
            <div className="sticky top-0 bg-gray-900/95 backdrop-blur-md border-b border-gray-800 p-4 sm:p-6 z-10">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="relative flex-shrink-0">
                      <div className="p-2 sm:p-3 bg-gradient-to-br from-gray-800 to-gray-700 rounded-lg shadow-sm">
                        {selectedProject.type === 'video' ? <Play size={20} className="text-gray-300" /> : <Globe size={20} className="text-gray-300" />}
                      </div>
                ...
                <button
                  ...
                  className="flex-shrink-0 p-2 sm:p-3 hover:bg-gray-800 rounded-lg transition-all duration-200 hover:rotate-90 transform group"
                >
                  <X size={20} className="group-hover:text-red-500 transition-colors text-gray-400" />
                </button>
              </div>
            </div>
              ...
              <div className="p-4 sm:p-6 space-y-6 sm:space-y-8">
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3 animate-fade-in-delayed">
                  {selectedProject.tags.map((tag, index) => (
                    <span
                      key={index}
                      className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-gray-800 to-gray-700 text-gray-300 rounded-full text-xs sm:text-sm font-medium hover:from-gray-700 hover:to-gray-600 transition-all duration-300 cursor-default shadow-sm"
                    >
                      <Tag size={12} />
                      {tag}
                    </span>
                ...
                <article className="prose prose-sm prose-slate max-w-none">
                  {[
                    ...
                    <section key={index} className="mb-4 sm:mb-6">
                      ...
                      <div className="bg-gradient-to-r from-gray-800 to-gray-900 p-3 sm:p-4 rounded-lg border border-gray-700 shadow-sm">
                        <p className="text-gray-300 leading-relaxed text-xs sm:text-sm md:text-base">{section.content}</p>
                      </div>
                    </section>
                  ))}
                  ...
                  <section className="mb-4 sm:mb-6">
                    ...
                    <div className="grid gap-2 sm:gap-3">
                      {selectedProject.blogContent.features.map((feature, index) => (
                        <div 
                          key={index} 
                          className="flex items-start gap-2 p-2 sm:p-3 bg-gray-800 border border-gray-700 rounded-lg hover:border-gray-600 hover:shadow-md transition-all duration-300"
                        >
                          <div className="w-1 h-1 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full mt-2 flex-shrink-0"></div>
                          <span className="text-gray-300 leading-relaxed text-xs sm:text-sm md:text-base">{feature}</span>
                        </div>
                      ))}
                    </div>
                  </section>
                  ...
                  <section className="mb-4 sm:mb-6">
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      {selectedProject.blogContent.technologies.map((tech, index) => (
                        <div 
                          key={index} 
                          className="flex items-start gap-2 sm:gap-3 p-2 sm:p-3 bg-gradient-to-r from-gray-800 to-gray-900 border border-gray-700 rounded-lg hover:from-gray-700 hover:to-gray-800 hover:border-gray-600 hover:shadow-md transition-all duration-300"
                        >
                          <div className="w-1 h-1 bg-gradient-to-r from-green-500 to-blue-500 rounded-full mt-2 flex-shrink-0"></div>
                          <span className="text-gray-300 leading-relaxed font-medium text-xs sm:text-sm md:text-base">{tech}</span>
                        </div>
                      ))}
                    </div>
                  </section>
                  ...
                  <section className="mb-4 sm:mb-6">
                    ...
                    <div className="bg-gradient-to-r from-emerald-900 to-teal-900 p-3 sm:p-4 rounded-lg border border-emerald-700 shadow-sm">
                      <p className="text-gray-300 leading-relaxed text-xs sm:text-sm md:text-base">{selectedProject.blogContent.outcome}</p>
                    </div>
                  </section>
                  ...
                  <section className="mb-4 sm:mb-6">
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      {selectedProject.blogContent.lessons.map((lesson, index) => (
                        <div 
                          key={index} 
                          className="flex items-start gap-2 sm:gap-3 p-2 sm:p-3 bg-gradient-to-r from-amber-900 to-orange-900 border border-amber-700 rounded-lg hover:from-orange-900 hover:to-amber-900 hover:shadow-md transition-all duration-300"
                        >
                          <div className="w-1 h-1 bg-gradient-to-r from-amber-500 to-orange-500 rounded-full mt-2 flex-shrink-0"></div>
                          <span className="text-gray-300 leading-relaxed text-xs sm:text-sm md:text-base">{lesson}</span>
                        </div>
                      ))}
                    </div>
                  </section>
                ...
                {/* Action Buttons */}
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-6 sm:pt-8 border-t border-gray-800">
                  {selectedProject.type === 'video' && selectedProject.youtubeId && (
                    <a
                  ...
                  {selectedProject.type === 'web' && selectedProject.url && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center gap-3 bg-gradient-to-r from-white to-gray-100 text-black px-6 sm:px-8 py-3 sm:py-4 rounded-lg font-semibold hover:from-gray-100 hover:to-white hover:scale-105 hover:shadow-xl transition-all duration-300 transform group text-sm sm:text-base"
                    >
                  ...
                  <button
                    onClick={handleCloseBlog}
                    className="flex items-center justify-center gap-3 border-2 border-gray-600 text-gray-300 px-6 sm:px-8 py-3 sm:py-4 rounded-lg font-semibold hover:border-gray-400 hover:bg-gray-800 hover:scale-105 transition-all duration-300 transform group text-sm sm:text-base"
                  >
